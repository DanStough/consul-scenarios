...
consul connect envoy -ignore-envoy-compatibility -gateway mesh -expose-servers -register -- -l critical &
...
CONSUL_HTTP_ADDR=localhost:9500 consul connect envoy -ignore-envoy-compatibility -sidecar-for static-client -address '{{ GetInterfaceIP "$NETWORK_INTERFACE" }}:19001' -admin-bind localhost:19004 -- -l critical &
consul connect envoy -ignore-envoy-compatibility -sidecar-for static-server -address '{{ GetInterfaceIP "$NETWORK_INTERFACE" }}:8446' -admin-bind localhost:19005 -- -l critical &
